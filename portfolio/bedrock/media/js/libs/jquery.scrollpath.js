	...
	var	PREFIX =  "-" + getVendorPrefix().toLowerCase() + "-",
		...
		methods = {
			...
			init: function( options ) {
				...
				initCanvas();
				initScrollBar();
                scrollToStartingPoint(settings.initComplete);
...
//					"keydown": keyHandler,
	...
	function animateSteps ( steps, duration, easing, callback ) {
		...
		var frames = ( duration / 1000 ) * FPS,
			...
			interval = setInterval(function() {
				easedSteps = Math.round( ($.easing[easing] || $.easing.swing)( ++currentFrame / frames, duration / frames * currentFrame, 0, steps, duration) * steps / 50 ) * 50;
				nextStep = wrapStep( startStep + easedSteps);
				if (currentFrame >= frames || paused) {
    ...
    function scrollToStartingPoint(onComplete) {
        isAnimating = true;
        var stepParam = 0;
        cb = pathList[ stepParam ].callback;
        var spLeft = {'left': -(pathList[ stepParam ].x - $( window ).width() / 2)};
        var spTop = {'top': -(pathList[ stepParam ].y - $( window ).height() / 2)};
        $(element).animate(spLeft, 1000, 'linear').animate(spTop, 1500, 'linear', function() {
            if (onComplete) {
                onComplete();
            }
            isAnimating = false;
	...
	function scrollToStep( stepParam, fromAnimation ) {
        ...
        if (settings.onMove) {
            settings.onMove(stepParam, step - prevStep);
        }
		var cb;
